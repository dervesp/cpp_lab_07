...
BOOST_AUTO_TEST_SUITE(findmax_function_)
	BOOST_AUTO_TEST_CASE(cannot_find_maximum_in_empty_array)
	{
		...
		{
			ConstCharArray charArrayMax = "Deep Thought";
			std::vector<ConstCharArray> emptyCharArrayVector = {};
			BOOST_CHECK(!FindMax(emptyCharArrayVector, charArrayMax));
			BOOST_CHECK_EQUAL(charArrayMax, "Deep Thought");
		}
	}
	BOOST_AUTO_TEST_CASE(can_find_maximum_in_nonempty_array)
	{
		...
		{
			ConstCharArray charArrayMax = "aa";
			const std::vector<ConstCharArray> charArrayVector = { "aa", "ca", "aa", "ba", "la", "aa"};
			BOOST_CHECK(FindMax(charArrayVector, charArrayMax));
			BOOST_CHECK_EQUAL(charArrayMax, "la");
		}
	}
